...
import jp.justincase.jackson.kotlin.numeric.NumericDecoder
...
object NumericDeserializerModifier : BeanDeserializerModifier() {
  ...
  fun <T : Any> modifyCovariantDeserializer(
      ...
      (sequenceOf(beanDesc.beanClass.kotlin) + beanDesc.beanClass.kotlin.allSuperclasses.asSequence())
          .mapNotNull { it.companionObjectInstance as? NumericDecoder<Any> }
          .map {
            @Suppress("UnstableApiUsage")
            val typeToken = TypeToken
                .of(it.javaClass)
                .getSupertype(NumericDecoder::class.java)
                .resolveType(NumericDecoder::class.java.typeParameters.first())
